...
bool I2c::init() {
    ...
    for (size_t i = 0; i <= inUse.size(); i++) {
        ...
        if (!inUse[int(peripheral)])
            continue;
        LOG_DEBUG("I2c", "Peripheral $0 in use", int(peripheral) + 1);
...
bool I2c::transmit(Peripheral peripheral, Address address, uint8_t* data, uint16_t len) {
    return HAL_I2C_Master_Transmit(getHandle(peripheral), address, data, len, timeout) == HAL_OK;
}

bool I2c::receive(Peripheral peripheral, Address address, uint8_t* data, uint16_t len) {
    return HAL_I2C_Master_Receive(getHandle(peripheral), address, data, len, timeout) == HAL_OK;
}
